...
<form method="POST" action="{{ route('submit-form') }}">
  ...
  @if(session('borrow'))
    ...
    <input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp"  placeholder="{{$need['product_name']}}">
    <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
  </div>
  
  <div class="col-12">
    <label for="inputAddress" class="form-label">Book Name</label>
    <input type="text" class="form-control" id="inputAddress" placeholder="{{$need['product_name']}}">
  </div>
  <div class="col-12">
    <label for="inputAddress" class="form-label">Host</label>
    <input type="text" class="form-control" id="inputAddress" placeholder="Host name" disable>
  </div>
  <div class="col-12">
    <label for="inputAddress" class="form-label">Cost</label>
    <input type="number" class="form-control" id="inputAddress" placeholder="$20" disable>
  ...
  <!--<label for="date">Select a Start Date:</label>
    ...
    <input type="date" id="date" name="date" value="{{ old('date', $futureDate) }}">
